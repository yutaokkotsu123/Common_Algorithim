...
void printData(DoVat dsdv[], int n)
{
    for (int i = 0; i < n; i++)
    {
        printf("%d %d %d %s %.2f | Phuong an la: %d\n", dsdv[i].TL, dsdv[i].GT, dsdv[i].SL, dsdv[i].TenDoVat, dsdv[i].DG, dsdv[i].PA);
    }
    printf("\n");
}
...
void Greedy(DoVat dsdv[], int n, int W, int *TGT, int *TL){
    int TLConLai = W;
    for (int i=0;i<n;i++){
        int So_Luong_Do_Vat_Co_The_Chon = min(TLConLai/dsdv[i].TL, dsdv[i].SL);

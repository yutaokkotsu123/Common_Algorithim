...
void readData(DoVat ds_vat[], int *n, int *W)
{
	...
	if (f != NULL)
	{
		...
		while (!feof(f))
		{
			fscanf(f, "%d%d%[^\n]", &ds_vat[i].TL, &ds_vat[i].GT, &ds_vat[i].TenDoVat);
...
void PrintDSDV(DoVat ds_vat[], int n)
{
	...
	for (int i = 0; i < n; i++)
	{
		printf(" %2d %2d %2s %.2f | Phuong an la: %d\n", ds_vat[i].TL, ds_vat[i].GT, ds_vat[i].TenDoVat, ds_vat[i].DG, ds_vat[i].PA);
	}
	printf("\n");
}
...
{
	int j;
	int So_Do_Vat_Co_The_Chon = min( *TLConLai / dsdv[i].TL, 1);
